...
from op_templates.adl.flex_template import FlexTemplate
from op_templates.templates.op_template import CodeletTemplate
from op_templates.adl.operation import Operand, Loop, Compute, Transfer, Configure, Operation
...
class OperandDataflow:
    ...
    def add_read(self, cdlt: OperationInstance, operand: Operand):
        self.read_operand_names.append(operand.name)
        self.cdlt_read.append(cdlt.instance_id)

    def add_write(self, cdlt: OperationInstance, operand: Operand):
...
class CodeletProgram(object):
    ...
    @property
    def codelets(self) -> List[OperationInstance]:
    ...
    def add_codelet(self, cdlt: OperationInstance):
    ...
    def set_instruction_templates(self, cdlt: OperationInstance):
        for o in cdlt.ops:
            template = self.hag.get_operation_template(o)
            template_copy = [ft.template_copy() for ft in template]
            o.set_template(template_copy)

    def instantiate_instructions(self, cdlt: OperationInstance, fixed_val=None, verbose=True):
        ...
        for tname, t in self.codelet_templates.items():
            if node.op_name == tname and shapes == self.get_template_shapes(t):
                ...
                raise RuntimeError(f"Unable to match node operation to codelet with the same name:\n"
                                   ...
                                   f"OperationInstance: {tname}\n"
                                   f"OperationInstance shapes: {self.get_template_shapes(t)}\n")
    ...
    def add_operand_mapping(self, cdlt: OperationInstance, parent_node: pm.Node, node: pm.Node, operand: Operand, operand_type):
    ...
    def instantiate_all_operations(self, node_sequence, codelets, verbose=False):
        if verbose:
            print(f"\nInstantiating OperationInstance Operations")
    ...
    def finalize_memory(self, node_sequence, codelets: List[OperationInstance], verbose=False):
                ...
                for ml in o.unset_mem_locations:
                    ...
                    if loc_sizes[ml] > mem_node.size:
                        raise RuntimeError(f"Invalid storage capacity for codelet\n"
                                           f"OperationInstance: {cdlt.op_name} - {cdlt.cdlt_uid} --> {ml}\n"
                                           f"Operand: {o.name}\n"
